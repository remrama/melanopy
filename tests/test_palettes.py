"""diel_sweep (full-axis / teaching) and diel_diverging (signed-data) palettes.
...
def test_diel_sweep_shape_and_cmap():
    rgb = mp.diel_sweep()
    assert rgb.shape == (256, 3)
    assert rgb.min() >= 0.0 and rgb.max() <= 1.0
    cm = mp.diel_sweep(as_cmap=True)
    assert isinstance(cm, ListedColormap) and cm.name == "diel_sweep"


def test_diel_sweep_is_protective_to_alerting():
    m = mp.melanopic_ratio(mp.diel_sweep())
    assert m[0] < 0.5  # starts protective (warm, low melanopic)
    assert m[-1] > 1.2  # ends alerting (cool, high melanopic)


def test_diel_sweep_melanopic_linear():
    m = mp.melanopic_ratio(mp.diel_sweep())
...
def test_diel_sweep_is_ordered():
    # monotone lightness -> ordered / CVD-recoverable like the rest of the family
    assert np.all(np.diff(_oklab_L(mp.diel_sweep())) > 0)


def test_diel_diverging_neutral_centre():
    n = 257  # odd -> an exact centre sample at t = 0
    m = mp.melanopic_ratio(mp.diel_diverging(n=n))
...
def test_diel_diverging_arms_monotone_lightness():
    n = 257
    L = _oklab_L(mp.diel_diverging(n=n))
    c = n // 2
...
def test_new_maps_register():
    ...
    mp.register()
    assert "diel_sweep" in plt.colormaps()
    assert "diel_diverging" in plt.colormaps()
