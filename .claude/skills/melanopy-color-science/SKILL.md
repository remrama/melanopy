...
description: >-
  ...
  M/P pipeline, the Diel family's invariants (display white = 1.0, one shared monotone OKLab
...
| `generator.py`   | OKLab↔linear, gamut clamp, the shared lightness profile, the Diel family   |
...
- `melanopic_ratio` — *where* the map sits: luminance-weighted **mean** position (white = 1).
- `purity_sigma` — *how tightly* it sits: luminance-weighted **spread** of the per-pixel ratio.
...
carries the order), not the stronger "CVD-safe". `diel_diverging` distinguishes its arms by
