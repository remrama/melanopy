...
\newcommand{\mpruler}{%
  ...
  \end{tikzpicture}}
\input{figures/leaderboard.tex}       % defines \leaderboardrows (the generated table body)
\input{figures/leaderboard_note.tex}  % defines \leaderboardnote (caption footnote for marginal maps)
...
A single mean still hides the structure that matters, so the rater reports \textbf{two distinct quantities} (plus the raw per-position range; the full per-position profile behind both is plotted in Appendix~\ref{app:profiles}, Fig.~\ref{fig:profiles}):
