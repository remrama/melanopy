...
\par
Because a display's primaries $P_R, P_G, P_B$ are fixed, the melanopic content of any display colour collapses to a weighted sum of \textbf{three per-primary coefficients} --- the melanopic/photopic ratio of each RGB primary.
...
These three numbers are the entire display-dependent part of the model, regression-locked to the validated spectrum so the data and the numbers cannot silently drift apart.
...
This section states what Melanopy deliberately does \emph{not} assert (Section~\ref{sec:limitations}), how the contribution sits against four mature literatures (Section~\ref{sec:related-work}), and how every result is reproducible (Section~\ref{sec:reproducibility}).

\par
\textbf{Potential applications.}
The generator's $\alpha$ is most useful as a \emph{live control}: exposed as a slider, it lets a screen's colour temperature move along the melanopic axis during a session without leaving the app.
Our reference deployment is SMACC, a PyQt6/pyqtgraph sleep- and dream-research tool whose operators watch live EEG spectrograms overnight; $\alpha$ drives the large-fill colormaps (spectrogram, hypnogram density, topographic maps) and optionally the UI accent, while categorical line/scatter views need not react --- their emission is negligible (Section~\ref{sec:axis}).
Applying the adjustment at the \emph{data $\rightarrow$ colour} step, rather than as a global gamma/LUT warp like \textbf{f.lux} or OS Night Shift, preserves perceptual uniformity and CVD-recoverability by construction: a global framebuffer tint breaks both for every figure on screen, and a spectral shift without dimming does not reliably reduce melatonin suppression anyway~\cite{nagare2019nightshift}.
This is a deployment example, not a contribution --- the slider changes how the screen \emph{participates in} the room's circadian strategy, never an alertness dial.
