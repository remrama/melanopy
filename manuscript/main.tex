...
Small qualitative marks --- lines, points, glyphs --- emit negligibly regardless of their colour, so for them the axis is effectively \emph{``ignore it''}: any CVD-safe qualitative palette serves, and none needs to react to the regime.
...
The one exception is a mark that must sit \emph{over} a large fill, where it can be lost against the colormap or clash with it; for that case \textbf{melanopy} provides \texttt{CIRCADIA\_ACCENT}, a small high-chroma set chosen to stay clear of the family's colours across the whole axis and to remain distinct under simulated colour blindness.
