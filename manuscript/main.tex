...
\newcommand{\mptick}[2]{\draw[gray!50,line width=0.3pt] (#1,0)--(#1,0.5); % chktex 36
...
First, we adapt the melanopic/photopic ratio (M/P) --- an established lighting measure --- to colormaps: three per-primary coefficients give an M/P for any sRGB colour, normalized so display white (the relevant neutral, not daylight) equals~1, summarized per map by a mean and spread and validated via luox.
...
\par
We contribute: (i) a \textbf{method for rating colormaps} on the melanopic axis, resting on a per-display \emph{three-coefficient collapse} (all display-dependence reduces to three per-primary M/P numbers, so no runtime spectral integration) and a \emph{mean-plus-spread} decomposition (a single mean hides melanopic ``smearing'', which a luminance-weighted spread catches); (ii) a one-parameter \textbf{generator} that traverses the axis while holding perceptual uniformity and CVD-recoverability fixed; (iii) a \textbf{scored index} of colormaps in common use, which places them on the axis and confirms the gap the generator fills; and (iv) the open-source package \textbf{melanopy} that ships all three.
...
Small qualitative marks --- lines, points, glyphs --- emit negligibly regardless of their colour, so for them the axis is effectively \emph{``ignore it''}: a CVD-safe qualitative palette serves every circadian regime.
This split is a design principle, not a limitation: it tells us to spend the melanopic budget on \emph{sequential fills} and to keep qualitative colour a separate concern.
(\textbf{Melanopy} ships a CVD-safe \emph{neutral} qualitative palette for this case --- optimised for category separability, not melanopic content; it stays well-separated under simulated colour blindness, Fig.~\ref{fig:melanopic-colormaps}.)
Where qualitative colour should instead \emph{track} the regime --- themed protective/alerting sets, accent marks that sit over a Circadia fill, or an ordered sleep-stage palette --- \textbf{melanopy} provides regime-aware qualitative palettes too (Section~\ref{sec:applications}).

\begin{figure}[t]
    \centering
    \includegraphics[width=\columnwidth]{melanopic_colormaps.png}
    \caption{The area-weighted budget in practice. \emph{Top:} the protective endpoint Sodium under simulated dichromacy stays order-preserving --- lightness does the ordering work. \emph{Middle/bottom:} the CVD-safe qualitative palette stays well-separated under colour blindness, so one palette serves every circadian regime.}\label{fig:melanopic-colormaps} % chktex 12
\end{figure}
...
This ratio is not ours to define: $R$ is the established lighting quantity behind the \textbf{melanopic ratio} in the WELL Building Standard's equivalent melanopic lux ($\mathrm{EML}=L\times R$)~\cite{iwbi_well_v2}, resting on the $\alpha$-opic framework of Lucas et al.~\cite{lucas2014melanopsin}, and its computation has been the subject of explicit calls for consistency~\cite{miller2020mp}.
We \emph{adapt} this established measure to colormaps; we do not introduce it.
...
Lighting references this ratio to a natural white --- daylight in WELL's melanopic ratio ($R\approx 1.1$), D65 in mel-DER --- whereas we reference the display's own neutral, normalizing so display white $=1$ (\eqref{eq:mp}): for a self-luminous colormap the meaningful zero is the screen's white, not daylight.
...
A colour $\mathbf{c}=(r,g,b)$ in sRGB is linearized channel-wise ($c\in\{r,g,b\}$) by the sRGB transfer function,
\begin{equation}
  \tilde c =
  \begin{cases}
    c/12.92, & c \le 0.04045,\\
    \bigl((c+0.055)/1.055\bigr)^{2.4}, & c > 0.04045, % chktex 3
  \end{cases}
  \label{eq:eotf}
\end{equation}
...
\begin{equation}
  ...
  \label{eq:mp}
\end{equation}
with the white-point normalizer $R_{\mathrm{white}} = \sum_c w_c\,\mu_c$.
...
Written this way, $\mathrm{M/P}(\mathbf c)=(M/Y)/R_{\mathrm{white}}$ is the colour's raw melanopic ratio referenced to that of display white; \textbf{throughout, M/P is normalized to display white} (the field instead names its D65-referenced counterpart mel-DER, rather than subscripting the symbol).
...
\begin{align}
  \overline{\mathrm{M/P}}
    &= \frac{\sum_{i\in\mathcal E} Y_i\rho_i}{\sum_{i\in\mathcal E} Y_i}
       \quad\text{(mean)}, \label{eq:mean}\\
  \sigma
    &= \sqrt{\frac{\sum_{i\in\mathcal E} Y_i(\rho_i-\overline{\mathrm{M/P}})^2}% chktex 3
                  {\sum_{i\in\mathcal E} Y_i}}
       \quad\text{(spread)}. \label{eq:spread}
\end{align}
...
\begin{table*}[!t]
    ...
    \caption{\textbf{The melanopic leaderboard.} Every colormap in the study on the melanopic axis (display white = 1), with the two universal requirements alongside: the \emph{ramp}; the \emph{M/P axis} cell (dot = M/P mean, bar = per-panel band across the four display archetypes); the M/P mean; the spread $\sigma$; and \textbf{PU} / \textbf{CVD-rec.}\ pass/fail. PU = monotone CAM02-UCS lightness ($J'$) with step-size CoV $< 0.30$; CVD-rec.\ = perceived lightness stays monotone under simulated deuteranopia, protanopia, and tritanopia (Machado et al., severity 100) --- the same checks applied to the Circadia family in Section~\ref{sec:circadia}. $\bullet$~pass, $\circ$~fail; ${}^{*}$~marginal (within 10\% of the PU threshold, or a near-flat CVD lightness step). Filled/open glyphs are used rather than colour so the requirement columns stay legible under CVD. Rows ascend by M/P; the ranking is stable across panels (Spearman $\rho \geq 0.99$). \leaderboardnote}\label{tab:leaderboard} % chktex 12
...
The same displays also carry discrete marks that the area-weighted budget (Section~\ref{sec:axis}) exempts from the axis --- and where those marks should nonetheless read \emph{with} the regime, \textbf{melanopy} supplies regime-aware qualitative palettes.
The clearest case is the hypnogram itself: sleep stages are ordered by cortical activation (Wake $>$ REM $>$ N1 $>$ N2 $>$ N3), so a stage palette can walk the same OKLab geometry as the family, co-varying colour temperature (alerting Wake $\rightarrow$ protective N3, an emergent monotone melanopic ratio) with lightness (light Wake $\rightarrow$ dark N3, so deep sleep reads dark and the ordering survives dichromacy).
REM, whose EEG is wake-like, sits near the alerting end but hue-offset off the warm--cool spine so it stays distinct; artefact and unscored epochs are out-of-band greys.
The stage colour thus \emph{is} its place on the melanopic axis --- the axis used to carry data rather than merely scored against it (Fig.~\ref{fig:hypnogram}) --- while themed protective/alerting sets and Circadia-matched accent marks cover the same need for scatter, events, and annotations.

\begin{figure}[t]
    \centering
    \includegraphics[width=\columnwidth]{hypnogram.png}
    \caption{The hypnogram palette --- the melanopic axis used to \emph{carry} ordered data rather than score it. \emph{Top:} a night's stages coloured by the palette; colour is each stage's place on the axis, and lightness falls with sleep depth. \emph{Middle:} the five stages walk the axis, alerting Wake $\rightarrow$ protective N3, with a monotone melanopic ratio; artefact and unscored epochs are out-of-band greys. \emph{Bottom:} under simulated dichromacy the stage order survives, because lightness carries it.}\label{fig:hypnogram} % chktex 12
\end{figure}
...
\par
Neither building block is itself novel: the melanopic axis is mature in lighting, and the generic move of treating a display-physical property as a colormap-design axis --- with a rating method, an evaluation of existing schemes, and a perception-constrained generator --- has precedent in energy-aware visualization.
...
\item[melanopic ratio (M/P)] An established lighting quantity (melanopic-to-photopic ratio), here adapted to colours and normalized so display white $=1$; $<1$ is protective (warm), $>1$ alerting (cool).
